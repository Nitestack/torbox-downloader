...
let torboxApiToken, gamesWebhookUrl, othersWebhookUrl;

browser.storage.local
  .get(["torboxApiKey", "discordGamesWebhookURL", "discordOthersWebhookURL"])
  .then((result) => {
    for (const [id, value] of Object.entries(result)) {
      if (id === "torboxApiKey") torboxApiToken = value;
      else if (id === "discordGamesWebhookURL") gamesWebhookUrl = value;
      else if (id === "discordOthersWebhookURL") othersWebhookUrl = value;
    }
  });

browser.storage.onChanged.addListener((updatedItems, areaName) => {
  if (areaName !== "local") return;

  for (const [id, value] of Object.entries(updatedItems)) {
    if (id === "torboxApiKey") torboxApiToken = value.newValue;
    else if (id === "discordGamesWebhookURL") gamesWebhookUrl = value.newValue;
    else if (id === "discordOthersWebhookURL")
      othersWebhookUrl = value.newValue;
  }
});
...
browser.contextMenus.onClicked.addListener(({ menuItemId, linkUrl }) => {
  if (!torboxApiToken)
    return notify(
      "An error occurred",
      "Missing TorBox API Key! Please go to \"about:addons\", search for TorBox Downloader, click on the three dots, select 'Preferences' and set your preferences.",
    );
  if (menuItemId === "download-magnet") {
    if (!linkUrl || !linkUrl.match(/magnet:\?xt=urn:btih:[0-9a-fA-F]{40,}.*/i))
...
async function download_source(formData) {
  try {
    const response = await fetch(`${torboxApiUrl}/torrents/createtorrent`, {
      method: "POST",
      headers: {
